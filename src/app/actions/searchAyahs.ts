...
type Metadata = {
    surah: Record<string | number, SurahMetadata>;
};
...
export async function searchAyahs(query: string): Promise<SearchResult[]> {
    ...
    const mapToResult = (doc: { _id: number; text: string;[key: string]: unknown }, source: string) => {
        const gi = doc._id;
        let surahNumber = 1;
        let ayahNumber = gi;
    ...
    const results = [
        ...enDocs.map(d => mapToResult(d as unknown as { _id: number; text: string;[key: string]: unknown }, "English")),
        ...bnDocs.map(d => mapToResult(d as unknown as { _id: number; text: string;[key: string]: unknown }, "বাংলা"))
    ];
